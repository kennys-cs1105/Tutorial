...
价值函数的值是对未来奖励的预测，用来评估状态的好坏。价值函数中有一个折扣因子，在函数中定义为:

$$V_{\pi}(s) = \mathbb{E}_{\pi}[\sum_{k=0}^{\infty}\gamma ^{k}r_{t+k+1}| s_{t}=s]$$

期望$\mathbb{E}_{\pi}$的下标是$\pi$函数，函数值可反映我们在使用策略$\pi$时可以得到多少奖励。

还有一种价值函数：$Q$函数，包含两个变量：状态和动作，定义为：

$$Q_{\pi}(s,a) = \mathbb{E}_{\pi}[G_{t}| s_{t}=s, a_{t}=a] = \mathbb{E}_{\pi}[\sum_{k=0}^{\infty}\gamma ^{k}r_{t+k+1}| s_{t}=s, a_{t}=a]$$

当得到$Q$函数后，进入某个状态要采取的最优动作可以通过$Q$函数得到。


#### 模型

模型决定了下一步的状态。下一步的状态取决于当前的状态以及当前采取的动作。他由状态转移概率喝奖励函数两部分组成。状态转移概率定义为：

$$p_{s{s}'}^{a} = p(s_{t+1}={s}'|s_{t}=s, a_{t}=a)$$

奖励函数是指我们在当前状态才去了某个动作可以获得多大的奖励：

$$R(s,a) = \mathbb{E}[r_{t+1}|s_{t}=s, a_{t}=a]$$

策略、价值函数、模型三部分形成了马尔可夫决策过程。
...
根据智能体学习的实物不同，将智能体进行分类
- 基于价值的智能体，显式的学习价值函数，隐式的学习策略，策略是从学到的价值函数中推导出来的
- 基于策略的智能体，直接学习策略。

基于策略的智能体没有学习价值函数。将基于价值的智能体和基于策略的智能体进行结合，称为演员-评论家智能体。这一类智能体将策略和价值函数都学习，通过两者的交互得到最佳的动作。

#### 基于模型的智能体与基于无模型的智能体

智能体可以通过到底有没有学习环境模型对智能体进行分类
- 基于模型的智能体，智能体可以通过学习环境模型来预测下一个状态和奖励。
- 基于无模型的智能体，智能体不能通过学习环境模型来预测下一个状态和奖励。
